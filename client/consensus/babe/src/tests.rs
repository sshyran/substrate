...
async fn revert_prunes_epoch_changes_and_removes_weights() {
	...
	let mut propose_and_import_blocks_wrap = |parent_id, n| {
		...
		let proposer_factory_ref = &mut proposer_factory;
		let block_import_ref = &mut block_import;
		async move {
			propose_and_import_blocks(
				client_ref,
				proposer_factory_ref,
				block_import_ref,
				parent_id,
				n,
			)
	...
	//     \                   to #10
	//      *-----E(#7)---#11                                          < fork #1
	let canon = propose_and_import_blocks_wrap(BlockId::Number(0), 21).await;
	let fork1 = propose_and_import_blocks_wrap(BlockId::Hash(canon[0]), 10).await;
	let fork2 = propose_and_import_blocks_wrap(BlockId::Hash(canon[7]), 10).await;
	let fork3 = propose_and_import_blocks_wrap(BlockId::Hash(canon[11]), 8).await;
...
async fn revert_not_allowed_for_finalized() {
	...
	let mut propose_and_import_blocks_wrap = |parent_id, n| async {
		propose_and_import_blocks(
			&client,
			&mut proposer_factory,
			&mut block_import,
	...
	let canon = propose_and_import_blocks_wrap(BlockId::Number(0), 3).await;
...
async fn importing_epoch_change_block_prunes_tree() {
	...
	let canon = propose_and_import_blocks_wrap(BlockId::Number(0), 30).await;

	// Create the forks
	let fork_1 = propose_and_import_blocks_wrap(BlockId::Hash(canon[0]), 10).await;
	let fork_2 = propose_and_import_blocks_wrap(BlockId::Hash(canon[12]), 15).await;
	let fork_3 = propose_and_import_blocks_wrap(BlockId::Hash(canon[18]), 10).await;
	...
	propose_and_import_blocks_wrap(BlockId::Hash(client.chain_info().best_hash), 7).await;
	...
	propose_and_import_blocks_wrap(BlockId::Hash(client.chain_info().best_hash), 8).await;
...
async fn obsolete_blocks_aux_data_cleanup() {
	...
	let fork1_hashes = propose_and_import_blocks_wrap(BlockId::Number(0), 4).await;
	let fork2_hashes = propose_and_import_blocks_wrap(BlockId::Number(0), 2).await;
	let fork3_hashes = propose_and_import_blocks_wrap(BlockId::Number(3), 2).await;
